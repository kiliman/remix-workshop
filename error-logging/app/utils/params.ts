...
function getParamsInternal<T>(
  ...
  | [{ [key: string]: string }, T, { [key: string]: string }]
  ...
  if (result.success) {
    return [null, result.data as T, fields]
  } else {
    let errors: any = {}
...
export function getParamsOrThrow<T extends ZodType<any, any, any>>(
  params: URLSearchParams | FormData | Record<string, string | undefined>,
  schema: T,
) {
  type ParamsType = z.infer<T>
  const [errors, data] = getParamsInternal<ParamsType>(params, schema)
  if (errors) {
...
export function getSearchParamsOrThrow<T extends ZodType<any, any, any>>(
  request: Pick<Request, 'url'>,
  schema: T,
) {
  type ParamsType = z.infer<T>
  let url = new URL(request.url)
  const [errors, data] = getParamsInternal<ParamsType>(url.searchParams, schema)
  if (errors) {
    throw new Response('Bad Request', { status: 400 })
  }
  return data
